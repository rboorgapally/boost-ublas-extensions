#ifndef _BOOST_UBLAS_RESHAPE_MATRIX_FROM_VECTOR_
#define _BOOST_UBLAS_RESHAPE_MATRIX_FROM_VECTOR_
...
namespace boost { namespace numeric { namespace ublas {
	...
	template<class M>
	class vector_reshape :
		public matrix_expression<vector_reshape<M> > {

			typedef vector_reshape<M> self_type;
	public:
		...
		BOOST_UBLAS_INLINE
			vector_reshape(matrix_type &data):
		matrix_expression<self_type> (),
			data_ (data), num_rows (1), num_cols (data.size()) {}
		BOOST_UBLAS_INLINE
			vector_reshape (const vector_reshape &m):
		matrix_expression<self_type> (),
			data_ (m.data_), num_rows (m.num_rows), num_cols (m.num_cols) {}
		BOOST_UBLAS_INLINE
			vector_reshape (matrix_type &data, size_type rows, size_type cols):
		matrix_expression<self_type> (),
		...
		// Assignment
		BOOST_UBLAS_INLINE
			vector_reshape &operator = (const vector_reshape &m) {
				matrix_assign<scalar_assign> (*this, m);
				return *this;
		}
		BOOST_UBLAS_INLINE
			vector_reshape &assign_temporary (vector_reshape &m) {
				*this = m;
				return *this;
		}
		template<class AE>
		BOOST_UBLAS_INLINE
			vector_reshape &operator = (const matrix_expression<AE> &ae) {
				matrix_assign<scalar_assign> (*this, matrix<value_type> (ae));
				return *this;
		}
		template<class AE>
		BOOST_UBLAS_INLINE
			vector_reshape &assign (const matrix_expression<AE> &ae) {
		...
		template<class AE>
		BOOST_UBLAS_INLINE
			vector_reshape &plus_assign (const matrix_expression<AE> &ae) {
		...
		template<class AE>
		BOOST_UBLAS_INLINE
			vector_reshape &minus_assign (const matrix_expression<AE> &ae) {
				matrix_assign<scalar_minus_assign> (*this, ae);
				return *this;
		}
		template<class AT>
		BOOST_UBLAS_INLINE
			vector_reshape& operator *= (const AT &at) {
		...
		// Closure comparison
		BOOST_UBLAS_INLINE
			bool same_closure (const vector_reshape &ba) const {
				return (*this).data ().same_closure (ba.data ());
		}

		// Swapping
		BOOST_UBLAS_INLINE
			void swap (vector_reshape &m) {
				if (this != &m) {
					BOOST_UBLAS_CHECK (num_rows == m.num_rows, bad_size ());
					BOOST_UBLAS_CHECK (num_cols == m.num_cols, bad_size ());
					matrix_swap<scalar_swap> (*this, m);
				}
		}
		BOOST_UBLAS_INLINE
			friend void swap (vector_reshape &m1, vector_reshape &m2) {
				m1.swap (m2);
		}
	...
	public:
		...
		class const_iterator1:
			public container_const_reference<vector_reshape>,
...
#ifndef BOOST_UBLAS_USE_INDEXED_ITERATOR
		class iterator1:
			public container_reference<vector_reshape>,
			public random_access_iterator_base<typename iterator_restrict_traits<
	...
	template<class V>
	BOOST_UBLAS_INLINE
		vector_reshape<V> vec_resh(V& data) {
			return vector_reshape<V>(data);
	};

	template<class V>
	BOOST_UBLAS_INLINE
		vector_reshape<V> vec_resh(V& data,int r,int c){
			return vector_reshape<V>(data,r,c);
	};

	template<class V>
	BOOST_UBLAS_INLINE
		const vector_reshape<const V> vec_resh(const V& data) {
			return vector_reshape<const V>(data);
	};
