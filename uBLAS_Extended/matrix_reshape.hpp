#ifndef _BOOST_UBLAS_RESHAPE_MATRIX_FROM_MATRIX_
#define _BOOST_UBLAS_RESHAPE_MATRIX_FROM_MATRIX_
...
namespace boost { namespace numeric { namespace ublas {
	...
	//Reshape matrix into another matrix 
	template<class M>
	class matrix_reshape:
		public matrix_expression<matrix_reshape<M> > {

			typedef matrix_reshape< M > self_type;
	public:
		...
		BOOST_UBLAS_INLINE
			matrix_reshape (matrix_type &data):
		matrix_expression<self_type> (),
			data_ (data), num_rows(data.size1()), num_cols(data.size2()) {}
		BOOST_UBLAS_INLINE
			matrix_reshape (const matrix_reshape &m):
		matrix_expression<self_type> (),
			data_ (m.data_), num_rows(m.num_rows), num_cols(m.num_cols) {}
		BOOST_UBLAS_INLINE
			matrix_reshape (matrix_type &data, size_type rows, size_type cols):
		matrix_expression<self_type> (),
			data_ (data), num_rows (rows), num_cols (cols) {}
		...
		BOOST_UBLAS_INLINE
			const_reference operator () (size_type i, size_type j) const {
				...
				return data () ((i*num_cols+j)/data_.size2(), (i*num_cols+j)%data_.size2());
		}
		BOOST_UBLAS_INLINE
		...
		// Assignment
		BOOST_UBLAS_INLINE
			matrix_reshape &operator = (const matrix_reshape &m) {
				matrix_assign<scalar_assign> (*this, m);
				return *this;
		}
		BOOST_UBLAS_INLINE
			matrix_reshape &assign_temporary (matrix_reshape &m) {
				*this = m;
				return *this;
		}
		template<class AE>
		BOOST_UBLAS_INLINE
			matrix_reshape &operator = (const matrix_expression<AE> &ae) {
				matrix_assign<scalar_assign> (*this, matrix<value_type> (ae));
				return *this;
		}
		template<class AE>
		BOOST_UBLAS_INLINE
			matrix_reshape &assign (const matrix_expression<AE> &ae) {
		...
		template<class AE>
		BOOST_UBLAS_INLINE
			matrix_reshape &plus_assign (const matrix_expression<AE> &ae) {
		...
		template<class AE>
		BOOST_UBLAS_INLINE
			matrix_reshape &minus_assign (const matrix_expression<AE> &ae) {
				matrix_assign<scalar_minus_assign> (*this, ae);
				return *this;
		}
		template<class AT>
		BOOST_UBLAS_INLINE
			matrix_reshape& operator *= (const AT &at) {
		...
		// Closure comparison
		BOOST_UBLAS_INLINE
			bool same_closure (const matrix_reshape &sa) const {
				return (*this).data ().same_closure (sa.data ());
		}

		// Swapping
		BOOST_UBLAS_INLINE
			void swap (matrix_reshape &m) {
				if (this != &m)
					matrix_swap<scalar_swap> (*this, m);
		}
		BOOST_UBLAS_INLINE
			friend void swap (matrix_reshape &m1, matrix_reshape &m2) {
				m1.swap (m2);
		}
	...
	public:
		...
		class const_iterator1:
			public container_const_reference<matrix_reshape>,
...
#ifndef BOOST_UBLAS_USE_INDEXED_ITERATOR
		class iterator1:
			public container_reference<matrix_reshape>,
			public random_access_iterator_base<typename iterator_restrict_traits<
	...
	template<class V>
	BOOST_UBLAS_INLINE
		matrix_reshape<V> mat_resh(V& data) {
			return matrix_reshape<V>(data);
	};

	template<class V>
	BOOST_UBLAS_INLINE
		matrix_reshape<V> mat_resh(V& data,int r,int c){
			return matrix_reshape<V>(data,r,c);
	};

	template<class V>
	BOOST_UBLAS_INLINE
		const matrix_reshape<const V> mat_resh(const V& data) {
			return matrix_reshape<const V>(data);
	};

}}}

#endif //_BOOST_UBLAS_RESHAPE_MATRIX_FROM_MATRIX_
